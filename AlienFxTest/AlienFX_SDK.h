...
#define ZeroMemory(v,l) memset(v, 0, l)
	...
	public:
        ...
        static void HidInputCallback(void * context, IOReturn result, void * sender, IOHIDReportType type, uint32_t reportID, uint8_t * report, CFIndex reportLength);
        
        bool HidDSetFeature(byte* buffer, int length);
        bool HidDSetOutputReport(byte* buffer, int length);
        bool HidDGetFeature(byte* buffer, int length);
        bool HidDGetInputReport(byte* buffer, int& length);
		...
		bool ToggleState(BYTE brightness, vector <mapping>* mappings = NULL, bool power = false);
